...
| D6 | **`tieBreak` corrected to `["C2","C3","C10","C1","C4","C6","C5","C7"]`** — C5 before C7. | The Diagnostic Spec's declared array contradicted its own worked example C, which ties C5 and C7 at 4 and states C5 comes first. Transposing the last two entries makes all three worked examples pass with **zero code change**. It is also the better product answer: for an unmarried woman whose only signal was «मायके में ज़मीन है», C5 should be her first card. Recorded inline in `matrix.v2.json`. |
| D7 | **No «छोड़ें» skip control in the UI.** | Overrides PRD F-19 and Diagnostic Spec §2/§5. Escapes are preserved at flow level: S0's «सीधे कार्ड देखें», Q3(f) «इनमें से कोई बात सच नहीं है», Q4(e) «पता नहीं», Q5(e) «इनमें से कोई नहीं», and the back arrow. `deck()` remains skip-tolerant defensively — the acceptance checklist requires an all-skipped fixture, and the WhatsApp bot will import the same function. |
| D9 | **`translate="no"` on rights and diagnostic surfaces.** | Found in testing: Chrome silently machine-translates the page on an English-locale browser. It rewrote `<html lang>` from `hi` to `en`, wrapped every text node, and replaced «यह सही नहीं है» verdicts and statute references with unreviewed output — exactly what the legal gate exists to prevent, and we already ship an *authored* English track that is strictly better. It also protects the `lang` attribute that drives TTS voice selection (F-110 r7) and the `:lang(hi)` rule that caps Mukta emphasis at 700 rather than the Latin 800. Home and institutional pages stay translatable. |
| D10 | **The pink step-down rule.** | Found by contrast testing, not assumed. `--pink-600` is 4.75:1 on pure white but only **4.23:1 on `--bg`** (#F4F5F7), which fails AA. So: pink-600 as a **fill** anywhere (white text on it is 4.9:1), pink-600 as **text on `--surface` only**, and on any tinted ground pink text steps down to `--pink-700`. Same move the myScheme system made when its brand green could not carry white text. 49 contrast assertions lock it in. |
| D8 | **Explorer deck is gate-filtered then topped up.** | Closes a collision the source spec does not cover, and which survives removing the skip control: `Q1d · Q2a · Q3f · Q4a · Q5{all}` scores max 2 with no pins, firing explorer — but the fixed explorer deck `[C6,C7,C2,C1,C8]` contains C1, which her Q1=d gate excludes, returning four cards under a screen that says «ये 5 कार्ड आपके लिए हैं». Filtering then topping up from her own highest-scoring cards yields `[C6,C7,C2,C5,C8]`, and C5 is the card her answer actually pointed at. Deck is always exactly 5. |
